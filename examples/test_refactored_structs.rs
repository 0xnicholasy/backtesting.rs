use backtesting::{Order, OrderType, OrderSide, Position, Trade};
use backtesting::types::{DirectionalTrade, ProfitLoss, Executable, StopManagement, Closeable};
use chrono::Utc;
...
fn main() {
    ...
    if let Some(mut position) = order.to_position(100.0, fill_time) {
        ...
        println!("Should trigger SL at 105.0: {}", position.should_trigger_sl());
        println!("Should trigger TP at 105.0: {}", position.should_trigger_tp());
